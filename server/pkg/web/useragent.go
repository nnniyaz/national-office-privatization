// parse useragent string to get info about client details: os, browser etc.
// got from github.com/mileusna/useragent
package web
...
func (u *UserAgent) IsEmpty() bool {
	return u.Name == "" && u.Version == "" && u.OS == "" && u.OSVersion == "" && u.Device == "" &&
		!u.Mobile && !u.Tablet && !u.Desktop && !u.Bot && u.URL == "" && u.String == ""
}
	...
	addToken := func() {
		if buff.Len() != 0 {
			...
			if _, ign := ignore[s]; !ign {
				...
				if val.Len() == 0 { // only if value don't exist
					var ver string
